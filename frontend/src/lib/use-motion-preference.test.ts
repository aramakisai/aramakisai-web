...
describe('useMotionPreference', () => {
  test('初期値は保存が無いときの OS 設定に従う (要件 21.7)', () => {
    stubMatchMedia(true);
    const { result } = renderHook(() => useMotionPreference());
    expect(result.current.reduced).toBe(true);
  });

  test('OS 設定が reduce でも保存値が no-preference なら reduce のままにする (要件 21.4)', () => {
  ...
  test('toggle で反転し、localStorage へ保存する (要件 21.6)', () => {
  ...
  test('reduced の値をルート要素の data-motion へ反映する (要件 21.4, 21.8)', () => {
    stubMatchMedia(false);
    const { result } = renderHook(() => useMotionPreference());
  ...
  test('保存済みの選択が無いとき、OS 設定の変更に追従する', () => {
  ...
  test('toggle で明示的に停止を選んだ後、OS 設定が変わっても値が変わらない', () => {
    const { fireChange } = stubMatchMedia(false);
    const { result } = renderHook(() => useMotionPreference());
  ...
  test('localStorage.getItem が例外を投げても OS 設定にフォールバックする', () => {
    stubMatchMedia(true);
  ...
  test('localStorage.setItem が例外を投げても toggle 自体は失敗しない', () => {
    stubMatchMedia(false);
  ...
  test('StrictMode の二重実行でも toggle 後の保存値は一貫する', () => {
    stubMatchMedia(false);
    const { result } = renderHook(() => useMotionPreference(), {
      wrapper: StrictMode,
    });
  ...
  test('SSR の初回描画 (常に false) を hydration でも引き継ぎ、警告を出さない (要件 21.7)', () => {
