...
| 4 | 3.3, 3.5, 5.2, 5.3, 6.5, 7.1 | 6 | 3.5 と 6.5 はインフラ側リポジトリだが対象ファイルが分かれる |
| 5 | 6.3, 7.2 | 2 | どちらも CMS が K8s 上に載った状態を前提とする |
...
- [ ] 3.5 Authentik に CMS 用のアプリケーションとプロバイダを定義する
  - `aramakisai-infra` の Terraform に prod の OIDC プロバイダとアプリケーションを追加する。
    命名と構成は既存の Directus 用定義を基準とする
  - リダイレクト URI に CMS のコールバックパスを登録する。
    ローカル開発用のリダイレクト URI も同じプロバイダに含める
...
- [ ] 6.5 インフラ側リポジトリに CMS の稼働定義を追加する
  ...
  - CNPG クラスタに Payload 用データベースを追加する。既存の Directus 用データベースには触れない
  - Deployment のリソース要求と上限を 1.2 の実測値に基づいて設定する
  - CMS のホスト名を DNS とトンネルの定義に追加し、外部から到達できるようにする
...
- [ ] 7.2 Directus を停止し Payload へ切り替える
  ...
  - _Depends: 3.5, 5.3, 6.5, 7.1_
  - _Requirements: 9.1, 9.2, 9.3_

- [ ] 8. 切り替え後のコンテンツ投入と検証
- [ ] 8.1 本番相当のコンテンツとメディアを投入する
  - レコード 5 件と画像 9 件を管理画面から投入する
...
- [ ] 9.3 (P) 不要になったシークレットと外部定義を撤去する
  ...
  - Directus 用のホスト名 (prod / staging) を DNS とトンネルの定義から削除する
