...
- **8.1 (コンテンツ投入)** — 管理画面からの手作業ではなく、prod DB へ port-forward した上で
  Payload の Local API を叩くスクリプト (`cms/scripts/seed-migration.ts`) で投入した。
  投入中に `media` テーブルに `prefix` カラムが存在しないスキーマドリフトを発見した。
  S3 有効時のみ Payload が要求するカラムで、ローカル (S3 未設定) では顕在化しなかった。
  additive なマイグレーション (`20260830_073945_add_media_prefix`) を追加して解消した。
  投入結果: media 9 件、announcements 1 件、festival_meta / page_home の 2 グローバル、pages 3 件。
