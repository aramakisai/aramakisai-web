...
- 高さは `collapsed` (内容の高さに合わせて縮んだ状態。実コンテンツを持たない状態の既定) / 中 (380px 相当) / 最大 (55vh) の 3 段のスナップ位置を持つ
- `collapsed` へ到達できるのは `AreaExhibitionListState` が `filtered` 以外 (`unselected` / `no-area` / `error`) のときのみ。これらは常に 1 行の案内文だけで実コンテンツを持たない。エリア選択時と検索結果表示時 (`filtered`) はリストが実コンテンツを持ち、内容の高さまで縮めても折りたたみとして意味をなさず長いリストがそのまま露出して破綻するため、下限を中 (380px) に固定する。すなわち `filtered` 以外は collapsed ⇔ 中 ⇔ 最大 の 3 段、`filtered` は 中 ⇔ 最大 の 2 段となる
...
- State model: 現在のスナップ位置 (`collapsed` / 中 / 最大) をローカルに持つ。`unselected` / `no-area` / `error` の既定は `collapsed` とする。`filtered` (エリア選択時・検索結果表示時) は `collapsed` を取り得ず、下限は中に固定する
