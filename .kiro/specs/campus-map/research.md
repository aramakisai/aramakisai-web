...
- **Key Findings**:
  ...
  - ズーム範囲は想定最大ビューポートを余白込みで覆う z17 を下限、OSM Carto が想定する最大縮尺の z19 を上限とする。1454 枚・約 3.83MB に収まり、この規模では PMTiles のような単一アーカイブ形式が解決する問題 (大量タイルの管理、Range リクエストによる部分取得) が発生しない。
...
- **Findings**:
  ...
  - 下限は z16。会場の全体像と周辺との位置関係を一目で掴むために z16 まで引けるようにする。z15 以下まで下げても周辺市街地が入るだけで会場案内としての情報は増えない。
  - z17 の範囲が覆う地理範囲を z18・z19 でも同じだけ覆う必要があるため、タイル数は z18 が z17 の 4 倍、z19 が 16 倍になる。
  - ファイルサイズは生成した webp タイルの実測値。openstreetmap-carto スタイルのタイルは色数が少なく、webp の可逆圧縮がよく効く。

  | ズーム | 枚数 | 平均サイズ/枚 (webp) | 総容量 |
  |---|---|---|---|
  | z16 | 25 | 約 7.7KB | 約 0.19MB |
  | z17 | 80 | 約 5.7KB | 約 0.45MB |
  | z18 | 285 | 約 3.6KB | 約 0.99MB |
  | z19 | 1064 | 約 2.1KB | 約 2.20MB |
  | 合計 | 1454 | — | 約 3.83MB |

- **Implications**: ズーム範囲を z16 (下限) 〜z19 (上限) に決定する。1454 枚・約 3.83MB はリポジトリへ資産としてコミットしても無理がない規模である。
...
- **Findings**:
  ...
  - Cloudflare Workers の static assets はファイル数上限が Free 20,000 / Paid 100,000、個別ファイルサイズ上限が 25MiB。1454 枚は余裕に収まる。
  - static assets へのリクエストは "free and unlimited" だが、存在しないパスは asset として解決されず Worker スクリプトに回り、Workers の課金対象になる。
- **Implications**: PMTiles が解決するのは「タイルが多すぎて個別ファイルとして扱えない」問題である。本件は 1454 枚であり、その問題が存在しない。個別 PNG を `frontend/public/` に置けば OpenNext がそのまま静的アセットとして配信し、Cloudflare の CDN に乗る。`pwa-offline` から見ても、個別 PNG は Cache API の `addAll()` にそのまま渡せる。ただし範囲外のタイル要求は 404 として Worker に回るため、クライアント側で要求そのものを発生させない設定 (TileLayer の `bounds`) が前提になる。
...
| Option | Description | Strengths | Risks / Limitations | Notes |
|--------|-------------|-----------|---------------------|-------|
| 個別 webp を静的アセット配信 | `frontend/public/map-tiles/{z}/{x}/{y}.webp` に置く | 追加依存ゼロ。Cloudflare CDN にそのまま乗る。Cache API に直接入れられる | リポジトリに約 3.83MB のバイナリ資産が入る | **採用** |
...
- **Selected Approach**: switch2osm が案内する `Overv/openstreetmap-tile-server` の Docker イメージを GitHub Actions 上で実行し、Geofabrik の群馬県 extract から会場周辺を切り出してレンダリングする。z16 を下限、z19 を上限とし (z17 で 2304×1792px を覆う 9×7 = 63 枚を範囲算出の基準とする)、合計 1454 枚を `frontend/public/map-tiles/{z}/{x}/{y}.webp` に資産としてコミットする。ワークフローは `workflow_dispatch` の手動実行とし、成果物を artifact として受け取る。
...
- **Trade-offs**: リポジトリに約 3.83MB のバイナリが入る。範囲やズーム幅を変えるにはワークフローの再実行とコミットが必要で、インポートからやり直すため 1 回あたりの所要時間が長い。ズーム 19 を超える拡大ができない。
- **Follow-up**: 日本語ラベルが正しく描画されることを目視で確認済み。
...
- **タイル資産のコミットによるリポジトリ肥大** — ズーム範囲 z16〜19 で約 3.83MB (1454 枚) をコミットしている。webp は delta 圧縮が効かず、再生成のたびに同量が履歴に積まれる。範囲変更は容量を再計測してから行い、原則として再生成しない運用とする。
